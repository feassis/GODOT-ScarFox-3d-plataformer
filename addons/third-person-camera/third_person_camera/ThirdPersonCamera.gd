...
@export var cameraSpeedRotateX: float = 10
...
func _update_camera_tilt() :
	...
	var tween = create_tween()
	tween.tween_property(_camera_spring_arm, "global_rotation_degrees:x", tilt_final_val, 0.1)


func _update_camera_horizontal_rotation(delta) :
	# TODO : inverse
	_camera_spring_arm.rotation_degrees.y = _camera_spring_arm.rotation_degrees.y - camera_horizontal_rotation_deg
	_camera_spring_arm.rotation_degrees.y = wrapf(_camera_spring_arm.rotation_degrees.y, -180, 180)
	#var tween = create_tween()
	#tween.tween_property(_camera_spring_arm, "global_rotation_degrees:y", camera_horizontal_rotation_deg * -1, 0.1).as_relative()
